...
class FCN32VGG:
    ...
    def build(self, rgb, train=False, num_classes=19, random_init_fc8=True,
              debug=False):
        """
    ...
    def get_conv_filter(self, filter_size, in_channels, out_channels, name):
        ...
        print('Layer name: %s' % name)
        print('Layer shape: %s' % str(shape))
        if ((name=="conv1_1"  or name=="conv1_2") and self.freez==True):
        ...
        if not tf.get_variable_scope().reuse:
            if not ((name=="conv1_1"  or name=="conv1_2") and self.freez==True):
