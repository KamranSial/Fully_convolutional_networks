...
class FCN32VGG:
    ...
    def build(self, rgb, freez , train=None, num_classes=19, random_init_fc8=True,debug=False):
        """
        ...
        self.conv1_1 = self._conv_layer(bgr, 3, 64, "conv1_1",freez[0])
        self.conv1_2 = self._conv_layer(self.conv1_1, 64, 64, "conv1_2",freez[1])
        self.pool1 = self._max_pool(self.conv1_2, 'pool1', debug)

        self.conv2_1 = self._conv_layer(self.pool1, 64, 128, "conv2_1",freez[2])
        self.conv2_2 = self._conv_layer(self.conv2_1, 128, 128, "conv2_2",freez[3])
        self.pool2 = self._max_pool(self.conv2_2, 'pool2', debug)

        self.conv3_1 = self._conv_layer(self.pool2, 128, 256, "conv3_1",freez[4])
        self.conv3_2 = self._conv_layer(self.conv3_1, 256, 256, "conv3_2",freez[5])
        self.conv3_3 = self._conv_layer(self.conv3_2, 256, 256, "conv3_3",freez[6])
        self.pool3 = self._max_pool(self.conv3_3, 'pool3', debug)

        self.conv4_1 = self._conv_layer(self.pool3, 256, 512, "conv4_1",freez[7])
        self.conv4_2 = self._conv_layer(self.conv4_1, 512, 512, "conv4_2",freez[8])
        self.conv4_3 = self._conv_layer(self.conv4_2, 512, 512, "conv4_3",freez[9])
        self.pool4 = self._max_pool(self.conv4_3, 'pool4', debug)

        self.conv5_1 = self._conv_layer(self.pool4, 512, 512, "conv5_1",freez[10])
        self.conv5_2 = self._conv_layer(self.conv5_1, 512, 512, "conv5_2",freez[11])
        self.conv5_3 = self._conv_layer(self.conv5_2, 512, 512, "conv5_3",freez[12])
    ...
    def _conv_layer(self, bottom, in_channels, out_channels, name,tnt=False):
        with tf.variable_scope(name) as scope:
            filt = self.get_conv_filter(3, in_channels, out_channels,name,tnt=tnt)
            conv = tf.nn.conv2d(bottom, filt, [1, 1, 1, 1], padding='SAME')

            
            conv_biases = self.get_bias(out_channels,name,tnt=tnt)
    ...
    def _fc_layer(self, bottom, name, num_classes=None,
        ...
        with tf.variable_scope(name) as scope:
            ...
            if name == 'fc6':
                ...
                conv_biases = self.get_bias(4096,name, num_classes=num_classes,tnt=False)
            elif name == 'score_fr':
                name = 'fc8'  # Name of score_fr layer in VGG Model
                filt = self.get_fc_weight_reshape(name, [1, 1, 4096, 19],
                                                  num_classes=num_classes)
                conv_biases = self.get_bias(19, name, num_classes=num_classes,tnt=False)
            else:
    ...
    def get_conv_filter(self, filter_size, in_channels, out_channels, name,tnt=False):
    ...
    def get_bias(self,out_channels,name, num_classes=None,tnt=False):
        if self.data_dict is not None and name in self.data_dict:
            ...
            if (tnt==True):
                var = tf.get_variable(name="biases", initializer=init, shape=shape, trainable=False)
                print(name + "bias Frozen")
            else:
